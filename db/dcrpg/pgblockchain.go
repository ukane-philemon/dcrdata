...
func (pgb *ChainDB) GetExplorerTx(txid string) *exptypes.TxInfo {
	...
	if isVote := tx.IsVote(); isVote || tx.IsTicket() {
		...
		if tx.IsTreasurySpend() {
			...
			if tx.TSpendMeta.VoteEnd > tipHeight {
			...
			if tipHeight >= tx.TSpendMeta.VoteStart { // started
				secTillVoteStart, err = pgb.BlockTimeByHeight(tx.TSpendMeta.VoteStart)
				if err != nil {
					log.Errorf("Error fetching tspend start block time: %v", err)
				}
				tx.TSpendMeta.VoteStartDate = time.Unix(secTillVoteStart, 0).UTC()
			}
			...
			if tx.TSpendMeta.Approved { // tspend is approved, may have been short-circuited
			...
			} else if tipHeight >= tx.TSpendMeta.VoteEnd { // tspend was rejected
				secTillVoteEnd, err = pgb.BlockTimeByHeight(tx.TSpendMeta.VoteEnd)
				if err != nil {
					log.Errorf("Error fetching tspend end block time: %v", err)
				}
				tx.TSpendMeta.VoteStartDate = time.Unix(secTillVoteEnd, 0).UTC()
			}
